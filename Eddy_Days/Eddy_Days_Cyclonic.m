...
% It will take any shapefile -- could be any GoM study area, or a buffer around the study area -- and make a polyshape object

cd '/Volumes/SeascapeLab/Limer/FGB_EddyScan_Data/FGB_Boundary_Buffers'; % Shapefile path
...
Eddy_Day = 0;
for day = 1:31
    for d = 1:size(cyc2015,2) % Replace with the right year 
        eddy = cyc2015(day,d); % Replace with the right year 
        eddy = cell2mat(eddy);
        if size(eddy)>1
    ...
    for r = 1:e
        eddy = cyc2015(day,r); % Replace with the right year 
        eddy = cell2mat(eddy);
